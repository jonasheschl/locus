...
AGENTS_TEMPLATE = """# Locus Wiki maintenance contract
...
def ensure_wiki_contract(workspace: Path) -> None:
    ...
    for path, content in (
        (workspace / "AGENTS.md", AGENTS_TEMPLATE),
...
class OperationManager:
    ...
    def _rebuild_index(self, operation_id: str) -> None:
        pages = self.database.fetch_all(
            """
            SELECT path, title, excerpt FROM notes
            WHERE space='wiki' AND path NOT IN ('wiki/index.md', 'wiki/log.md')
        ...
        if pages:
            ...
            lines.append("")
            for page in pages:
                description = page["excerpt"] or "Synthesized Wiki page."
